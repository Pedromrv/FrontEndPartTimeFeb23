...
`localhost/articles/create` => `Route::get('articles/create',['ArticleController::class','create']');`
...
We see that we send the data to the route **'/articles'** using the HTTP verb **POST** with references to the controller and the method **store()** **'ArticleController'**.
