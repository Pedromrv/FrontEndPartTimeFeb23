...
The goal is to save our todos in `localStorage`, so we have them whenever we come back to our app.
...
Add a button to clear the `todos` done. Remember if you have to change the state (the `todolist`) of your app, don't modify it directly. Which function do you have to use to modify the state ? 🤔
...
Now you can try to improve the design of your app to make this one attractive. And why not add some features ??

At the end, push your changes to GitHub, build and deploy the app on Netlify, and ping your coach on Discord so that he/she can check the code you wrote.
